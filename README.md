...
The experimental offline web cache has been withdrawn after a reported loading failure. Play online for now. Open `/restore.html` to unregister this game's web worker, clear only its asset caches and return to the game. `dist/sw.js` remains as a network-only recovery worker so existing installations can stop serving cached pages. Gameplay retains the slower pace and same-chapter retries. The native iPhone bundle remains offline-capable.
